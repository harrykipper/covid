...
to go
  ...
  if length tests-sym + length tests-oth > 0 [test-people]
  ...
  ask turtles with [infected? and (not hospitalized?)] [   ;; we could exclude those still in the incubation phase here. We don't, so that we produce a few false positives in the app
    ...
    [
      let situation "symptomatic-individual"
      if not isolated? [maybe-isolate situation]
      enter-queue situation
    ]
  ]
...
to hospitalize ;; turtle procedure
  ...
  ask tracing-neighbors with [should-test?] [
    let situation "app-contact-of-positive"
    if not isolated? [maybe-isolate situation]
    enter-queue situation
  ]
...
to test-people
  set tests-sym remove-duplicates tests-sym
  set tests-oth remove-duplicates tests-oth
  let n-sym length tests-sym
  let n-oth length tests-oth
  let stillToTest tests-remaining
  if tests-remaining >= n-sym [set stillToTest n-sym]
  repeat stillToTest [
    let thisguy item 0 tests-sym
    set tests-sym remove-item 0 tests-sym
    ask thisguy [get-tested]
  ]
  ;; If we are prioritizing the symptomatics then another list exists, so if we still have tests left, we parse that list as well.
  if prioritize-symptomatics? and tests-remaining > 0 [
    set stillToTest tests-remaining
    if tests-remaining > n-oth [set stillToTest n-oth]
    repeat stillToTest [
      let thisguy item 0 tests-oth
      set tests-oth remove-item 0 tests-oth
      ask thisguy [get-tested]
    ]
  ]
  ...
  ifelse infected? [
    ...
    if any? relatives [
      ask relatives [
        if should-isolate?
            [
             let situation "relation-of-positive"
              maybe-isolate situation
              enter-queue situation
        ]
      ]
    ]
...
345
Symptomatic
length remove-duplicates tests-sym
0
1
11
...
345
Other
length remove-duplicates tests-oth
17
1
11
...
export-network</setup>
  ...
  <experiment name="noSocialD" repetitions="20" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="show-layout">
      <value value="false"/>
  ...
  <experiment name="NOsymPriority" repetitions="20" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="show-layout">
      <value value="false"/>
